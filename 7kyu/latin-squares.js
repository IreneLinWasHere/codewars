...
function makeLatinSquare(n) {
    let result = []
    let arrOfNums = [...Array(n).keys()].map(i => i + 1)

    for (let i = 0; i < n; i++) {
        let right = arrOfNums.slice(0, n - i)
        let left = arrOfNums.slice(n - i)
        let tempArr = left.concat(right)
        result.push(tempArr)
    }
    return result
}

console.log(makeLatinSquare(n))